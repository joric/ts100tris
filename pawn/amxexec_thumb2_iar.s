;   amxexec_thumb2_iar.s
;   Abstract Machine for the "Pawn" language
;
;   Adapted to the IAR EWARM by Dark Simpson, 2012
;
;   This file uses the ARM assembler syntax. It uses standard
;   or ARM THUMB2 instructions. Define THUMB2 to use THUMB2 set.
;
;   It can be assembled for Big Endian environments by
;   defining BIG_ENDIAN; the default configuration is
;   Little Endian.
;
;   You will need to compile the standard AMX.C file with
;   AMX_ASM defined.
;
...
#include "pawnconf.h" ; Defines shared with amx.c

    NAME amxexec

    PUBLIC amx_exec_list
    PUBLIC amx_exec_run

    SECTION amxexec_data:CONST:NOROOT(2)
...
GETPARAM_P MACRO rx             ; the opcode/parameter pack should be in r12
    mov rx, r12, ASR #16        ; rx = r12 >> 16 (signed)
    ENDM

JUMPREL MACRO rtmp              ; rtmp = temp register to use, cc = condition code
      ldr rtmp, [r4], #-4       ; rtmp = [CIP], CIP -= 4 (restore CIP to start of instruction)
      add r4, r4, rtmp          ; CIP = CIP + [CIP] - 4
    ENDM

JUMPRELEQNE MACRO rtmp          ; rtmp = temp register to use
      itte eq
      ldreq rtmp, [r4], #-4     ; rtmp = [CIP], CIP -= 4 (restore CIP to start of instruction)
      addeq r4, r4, rtmp        ; CIP = CIP + [CIP] - 4
      addne r4, r4, #4          ; otherwise skip param
    ENDM

JUMPRELNEEQ MACRO rtmp          ; rtmp = temp register to use
      itte ne
      ldrne rtmp, [r4], #-4     ; rtmp = [CIP], CIP -= 4 (restore CIP to start of instruction)
      addne r4, r4, rtmp        ; CIP = CIP + [CIP] - 4
      addeq r4, r4, #4          ; otherwise skip param
    ENDM

JUMPRELLTGE MACRO rtmp          ; rtmp = temp register to use
      itte lt
      ldrlt rtmp, [r4], #-4     ; rtmp = [CIP], CIP -= 4 (restore CIP to start of instruction)
      addlt r4, r4, rtmp        ; CIP = CIP + [CIP] - 4
      addge r4, r4, #4          ; otherwise skip param
    ENDM

JUMPRELLEGT MACRO rtmp          ; rtmp = temp register to use
      itte le
      ldrle rtmp, [r4], #-4     ; rtmp = [CIP], CIP -= 4 (restore CIP to start of instruction)
      addle r4, r4, rtmp        ; CIP = CIP + [CIP] - 4
      addgt r4, r4, #4          ; otherwise skip param
    ENDM

JUMPRELGTLE MACRO rtmp          ; rtmp = temp register to use
      itte gt
      ldrgt rtmp, [r4], #-4     ; rtmp = [CIP], CIP -= 4 (restore CIP to start of instruction)
      addgt r4, r4, rtmp        ; CIP = CIP + [CIP] - 4
      addle r4, r4, #4          ; otherwise skip param
    ENDM

JUMPRELGELT MACRO rtmp          ; rtmp = temp register to use
      itte ge
      ldrge rtmp, [r4], #-4     ; rtmp = [CIP], CIP -= 4 (restore CIP to start of instruction)
      addge r4, r4, rtmp        ; CIP = CIP + [CIP] - 4
...
CHKMARGIN MACRO rtmp            ; rtmp = temp register to use
...
CHKHEAP MACRO rtmp              ; rtmp = temp register to use
    ldr rtmp, [r10, #amxHLW]
...
; ================================================================
    ...
    SECTION amxexec_code:CODE:NOROOT(2)
...
OP_JZER                         ; tested
    cmp r0, #0
    JUMPRELEQNE r11             ; if PRI == 0, jump
    NEXT

OP_JNZ                          ; tested
    cmp r0, #0
    JUMPRELNEEQ r11             ; if PRI != 0, jump
    NEXT
...
OP_JEQ                          ; tested
    cmp r0, r1
    JUMPRELEQNE r11             ; if PRI == ALT, jump
    NEXT

OP_JNEQ                         ; tested
    cmp r0, r1
    JUMPRELNEEQ r11             ; if PRI != ALT, jump
    NEXT

OP_JSLESS                       ; tested
    cmp r0, r1
    JUMPRELLTGE r11             ; if PRI < ALT (signed), jump
    NEXT

OP_JSLEQ                        ; tested
    cmp r0, r1
    JUMPRELLEGT r11             ; if PRI <= ALT (signed), jump
    NEXT

OP_JSGRTR                       ; tested
    cmp r0, r1
    JUMPRELGTLE r11             ; if PRI > ALT (signed), jump
    NEXT

OP_JSGEQ                        ; tested
    cmp r0, r1
    JUMPRELGELT r11             ; if PRI >= ALT (signed), jump
    NEXT
