...
/* output to listing (.LST) file */
void *pc_openlst(char *filename); /* read/write */
void pc_closelst(void *handle,int deletefile);
void pc_resetlst(void *handle);
int  pc_writelst(void *handle,const char *str);
char *pc_readlst(void *handle,char *target,int maxchars);

/* output to assembler (.ASM) file */
...
SC_FUNC void ffcase(cell value,int label,int newtable);
...
SC_FUNC int enablewarning(int number,int enable);
...
SC_VDECL char lstfname[];     /* preprocessor listing file name */
SC_VDECL char asmfname[];     /* intermediate (assembler) file name */
...
SC_VDECL int sc_lstfile;      /* create .LST file? */
...
SC_VDECL FILE *lstf;          /* lst file */
SC_VDECL FILE *asmf;          /* asm file */

SC_VDECL jmp_buf errbuf;      /* target of longjmp() on a fatal error */

#if !defined PAWN_NO_XREPORT
