...
#define VERSION_STR "4.0.4548"
#define VERSION_INT 0x0400

static void resetglobals(void);
static void initglobals(void);
static char *get_extension(char *filename);
static void setopt(int argc,char **argv,char *bname,char *ename,char *pname,
...
#if !defined PAWN_NO_SPF || defined PAWN_XMLDOC
...
int pc_compile(int argc, char *argv[])
{
  ...
  *inpfname='\0';
  ...
  setopt(argc,argv,binfname,errfname,incfname,reportname,codepage);
  ptr=get_extension(binfname);
  if (ptr!=NULL && (stricmp(ptr,".asm")==0 || stricmp(ptr,".lst")==0))
    error(111, "\".asm\" or \".lst\" can't be used as extension for binary output file!");
  else if (ptr!=NULL)
    set_extension(binfname,".amx",FALSE);
  else
    set_extension(binfname,".amx",TRUE);
  /* Set names for listing and asm files */
  strcpy(lstfname,binfname);
  set_extension(lstfname,".lst",TRUE);
  strcpy(asmfname,binfname);
  set_extension(asmfname,".asm",TRUE);
  //
  ...
  freading=TRUE;
  if (sc_lstfile) {
    lstf=(FILE*)pc_openlst(lstfname);
    if (lstf==NULL)
      error(101,lstfname);
  }
  asmf=(FILE*)pc_openasm(asmfname); /* first write to assembler file (may be temporary) */
  if (asmf==NULL)
    error(101,asmfname);
...
cleanup:
  ...
  if (errnum==0 && jmpcode==0) {
    assert(binf!=NULL);
    pc_resetasm(asmf);          /* flush and loop back, for reading */
    hdrsize=assemble(binf,asmf); /* assembler file is now input */
  } /* if */
  if (lstf!=NULL) {
    pc_closelst(lstf,!(sc_lstfile));
    lstf=NULL;
  } /* if */
  if (asmf!=NULL) {
    pc_closeasm(asmf,!(sc_asmfile));
    asmf=NULL;
  } /* if */
  if (binf!=NULL) {
  ...
  if (errnum==0 && strlen(errfname)==0) {
    ...
    if (pc_amxram>0 && (glb_declared+pc_stksize)*pc_cellsize>=(unsigned long)pc_amxram)
      flag_exceed=1;
    if ((sc_debug & sSYMBOLIC)!=0 || verbosity>=2 || stacksize>=(long)pc_stksize || flag_exceed) { // Stack size + 32 ???
...
static void initglobals(void)
{
  ...
  sc_lstfile=FALSE;     /* do not create .LST file */
...
#if defined PAWN_KEELOQ
  pc_cryptkey=0;
#endif

  lstfname[0]='\0';     /* lst file name */
  asmfname[0]='\0';     /* asm file name */
 ...
 */
#if !defined PAWN_NO_SPF
static void parserespf(char *filename,char *bname,char *ename,char *pname,
                       char *rname, char *codepage);
#endif

static void parseoptions(int argc,char **argv,char *bname,char *ename,char *pname,
                         char *rname, char *codepage)
{
  char str[_MAX_PATH];
  ...
  for (arg=1; arg<argc; arg++) {
    ...
    if (isoption) {
      ...
      case 'l':
        if (*(ptr+1)!='\0')
          about();
        sc_lstfile=TRUE;        /* skip second pass & code generation */
        break;
      case 'N':
        ...
        break;
      case 'o':
        strlcpy(bname,option_value(ptr),_MAX_PATH); /* set name of (binary) output file */
        break;
      case 'O':
      ...
      #if !defined PAWN_NO_SPF
        parserespf(&argv[arg][1],bname,ename,pname,rname,codepage);
      #endif
...
#define MAX_OPTIONS     200
  ...
  parseoptions(argc,argv,bname,ename,pname,rname,codepage);
  ...
  strcpy(pname,sDEF_PREFIX);
    ...
    if (sc_binpath[0]!='\0') {
      ...
      if (access(cfgfile,4)==0)
        parserespf(cfgfile,bname,ename,pname,rname,codepage);
      else if (found)
...
static void setcaption(void)
{
  pc_printf("Pawn compiler %s\n\nCopyright (c) 1997-2011, ITB CompuPhase\nModified by Dark Simpson, 2012\n\n",VERSION_STR);
}
...
static void about(void)
{
  if (strlen(errfname)==0) {
    ...
    pc_printf("         -a       additionally output assembler code in .asm file\n");
...
#endif
    pc_printf("         -l       additionally output preprocessed source code in .lst file\n");
    pc_printf("         -o<name> set name of binary output file\n");
    ...
    pc_printf("         -X<num>  abstract machine total size limit in bytes\n");
...
static void doswitch(void)
{
  ...
  ffcase(casecount,label,TRUE);
  /* generate the rest of the table */
  for (cse=caselist.next; cse!=NULL; cse=cse->next)
    ffcase(cse->value,strtol(cse->name,NULL,16),FALSE);
